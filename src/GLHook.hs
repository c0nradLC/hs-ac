...
import Global
    ( playerEntityPointerRef,
      ...
      loadedRef,
      originalSwapWindowFuncRef, dokillFunPtrRef, playerAimXAddressRef, playerAimYAddressRef, playerInCrosshairPtrRef )
import Data.IORef ( readIORef, writeIORef )
import Geom (worldToScreen, deltas, getDistance, getAngles)
            ...
            catMaybes <$> mapM (\botPointer -> do
                ...
                case mBotAddress of
                    Just botAddress -> do
                        ...
                        if botState == 0 then do --when bot is alive
                            ...
                            botIsVisible <- readIORef isVisibleFunPtrRef >>=
...
magnet pid localPlayer players = do
    mapM (\bot -> do
        if _team bot == _team localPlayer then return bot
        else do
...
aimbot pid localPlayer playersList = do
    -- get the closes visible enemy bot as the target
