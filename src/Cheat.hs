...
import External
import Foreign
import Geom (getAngles, worldToScreen)
import Global
...
patchGodMode pid gameModuleBase playerEntityPtr = do
  ...
  case mLocalPlayerAddr of
    Just localPlayerAddr -> do
      ...
      Mem.writeBytes pid (gameModuleBase + Offsets.codeCave) $ [0x49, 0xb8] ++ Mem.wordToLittleEndian localPlayerAddr 8
    Nothing -> return ()
  ...
  Mem.writeBytes pid ((gameModuleBase + Offsets.codeCave) + 0xd) $ [0x0f, 0x85] ++ Mem.wordToLittleEndian ((0x100000000 - (jumpOffsetAddr + 0x5)) - 0x5) 8
  ...
  Mem.writeBytes pid ((gameModuleBase + Offsets.codeCave) + 0x1e) $ 0xe9 : Mem.wordToLittleEndian ((0x100000000 - (jumpOffsetAddr + 0x15)) - 0x5) 8
